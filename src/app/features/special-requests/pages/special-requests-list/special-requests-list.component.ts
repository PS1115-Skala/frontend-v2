...
export class SpecialRequestsListComponent
  ...
  ngOnInit() {
    ...
    this.loadReservasEspeciales(
      !this.isLabF ? this.coreService.getuserId() : null
    );
    this.coreService
      .getAdminLabs()
  ...
  loadTrimestres() {
    this.coreService
      .getTrimesters()
      .subscribe((response) => (this.trimestres = response));
  }
  ...
  loadReservasEspeciales(laboratorio?: string, trimestre?: string) {
    ...
    this.specialRequestsService
      .getSpecialReservations(laboratorio, trimestre)
      .subscribe((response) => {
  ...
  onFormChange(event: any) {
    ...
    const trimestre =
      this.form.value.trimestre == "todos" ? null : this.form.value.trimestre;
  ...
  detailReserva(idReserva: number) {
    this.specialRequestsService
      .getSpecialReservations(null, null, idReserva)
      .subscribe((response) => {
        this.dialogRef = this.dialog.open(SpecialRequestsFormModalComponent, {
          width: "575px",
          data: {
            title: "Detalle Reserva Especial",
            laboratorios: this.laboratorios,
            datos: response,
          },
        });
  ...
  deleteReserva(idReserva: any) {
    this.dialogRef = this.dialog.open(DeleteAsignationModalComponent, {
      data: {
        title: "Atención",
        message: "Esta seguro que desea eliminar la reserva?",
      },
    });
    ...
    this.dialogRef.afterClosed().subscribe((result) => {
      if (result == "Si") {
        this.specialRequestsService
          .deleteSpecialReservation(idReserva)
          .subscribe(
            (response) => {
              this.coreService.successNotification(
                "Reserva especial eliminada exitosamente"
              );
              this.onFormChange({});
            },
            (error) => {
  ...
  nuevaReserva(values: any) {
    ...
    this.specialRequestsService
      ...
      .subscribe(
        (response) => {
          this.coreService.successNotification(
            "Reserva especial aceptada exitosamente"
          );
          this.onFormChange({});
        },
        (error) => {
