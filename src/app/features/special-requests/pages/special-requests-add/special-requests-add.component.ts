...
export class SpecialRequestsAddComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  consultaHistorico() {
    ...
    if (this.dataSource && this.dataSource.length > 0) { this.dataSource = []; }
    this.specialRequestsService.getUserSpecialReservations(this.coreService.getuserId())
    .pipe( finalize( () => {
      this.isLoading = false;
      this.isTableReady = true;
      this.dtTrigger.next();
